...
p_sagittal = polyfit(sagittal(:,2), sagittal(:,1), 6);

% define a barrier (skull) using the tanh function
W = max(axial,[],1);
W = abs(W(2));  % half width from temple to temple
H = max(sagittal, [],1);
H = W(1);       % height from eye brow to hairline
D = max(axial,[],1);
D = abs(D(2));  % depth of forehead

% Make a mesh inside the bone
xcoords = linspace(-W,W,Npts);
ycoords = linspace(-1.4*D,D,Npts);
zcoords = linspace(0,H, Npts);

[xmesh ymesh zmesh] = meshgrid(xcoords, ycoords, zcoords);

for bz = linspace(0,H, Npts);
    for bx = linspace(-W,W,Npts)
        
        % by = D*tanh(crv*(W - abs(bx))) ;
        % by = D-D* bx.^2 /W/W;
        by = polyval(p_axial, bx);
        
        %slfactory =  (H-bz)^0.4 / (H^0.4) ;
        slfactory = polyval(p_sagittal, bz) / polyval(p_sagittal, 0);
        by = by * slfactory ;
        
        slfactorx =  (H-bz)^0.3 / (H^0.3) ;
        bx = bx * slfactorx;
...
% Now do the back of the head (symmetric for now)
B2 = B;
B2(:,2,:) = -1.2*B2(:,2,:);
B = [B; B2];
...
sz_array = linspace(0,H2, Nelementsz);
slfactory = polyval(p_sagittal, sz_array);
slfactory = slfactory - min(slfactory);
slfactory = slfactory/max(slfactory);

for z = 1: Nelementsz;
    for sx = linspace(-W2,W2,Nelementsx)
        
        sy = D2*tanh(crv*(W2 - abs(sx))) ;
        sz = sz_array(z);
        
        % slfactory =  (H2-sz)^0.3 / (H2^0.3) ;
        slfactorx =  (H2-sz)^0.3 / (H2^0.3) ;
