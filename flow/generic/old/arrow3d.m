...
% 
% $Id: arrow3d.m 521 2013-04-24 19:10:38Z klitinas $
...
if strcmp(ltype,'line')
    ...
    for i=1:length(dis)
        ...
        hlines(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',[1,1,1]);
        % hlgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),[145,90,7]/255);
        hlgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),[255,255,255]/255);
        set(hlgrd(i), 'FaceAlpha', 0);
        set(hlgrd(i), 'EdgeColor', 'none');
    end
end
...
for i=1:length(dis)
    ...
    [rx,ry,rz] = rotatedata(xi,yi,zi,nvec(i,:),cosrang(i),[0,0,0]);
    cx=start(i,1)+rx;cy=start(i,2)+ry;cz=start(i,3)+rz;
    hheads(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',[1 1 1]);
    %Draw the underside of taper
    % hhgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),[7 86 145]/255);
    hhgrd(i)=patch(cx(1,:),cy(1,:),cz(1,:),[1 1 1]);
    set(hhgrd(i), 'EdgeColor', 'none');
end
