...
% 
% this version estimates the baseline separaterly as another parameter
%
...
[estimates , resnorm, res, ex, output]  = lsqnonlin(@kinetix_lsq,...
    est0, LB, UB, ...
    optvar,...
    t,parms, ...
    series);
...
fseries= estimates(1:end-1) + estimates(end);
t = [TR: TR: TR*(length(series))];
modeled = kinetix_lsq(estimates,t,parms);
...
fevent2 = [fevent-mean(fevent)  mean(fevent)];
...
modeled = kinetix_lsq(fseries2,t,parms);
...
[ev_estimates , resnorm, res, ex, output]  = lsqnonlin(@kinetix_lsq,...
    est0, LB, UB, ...
    optvar,...
    t,parms, ...
    evdata);
