...
%
% (4/2/2012)  this version is designed to simulated conditions at 7T and
% fast flows
...
if ~doBatch
    ...
    Gz_ss = 0.6;  % slice select gradient in G/cm.
    Gz_ave = 0.001;  % this is the average gradient between pulses
    
    % eta = 0.25;   % works for all locs if eta = 0.25!!
    % eta = ;   % works for all locs if eta = 0.25!!
    t_ramp = 0.5;  % ramp time in ms.
    ...
    t_rfrepeat = 1; % ms - this is the interval between pulses in terms of the scanner variable
end
%%
...
RF_spacing = t_rfrepeat ; % time between RF pulses in ms.
...
% alternative: use G_max and G_ave
A_Gz_ss2 = -(t_rfrepeat*Gz_ave - A_Gz_ss);
