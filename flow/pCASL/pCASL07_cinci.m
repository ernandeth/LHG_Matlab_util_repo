...
%
% (10/23/08) this version is for trying to hold the spins at their inverted
% state as long as possible:   pulse train is:  170 - 20 - (-20) - 20 - (-20)
...
if ~doBatch
    ...
    flip_ang = 11;
    isTag = 1;  % this means that the pulse's sign oscillates
    slomo =0;
...
%%
t_rfrepeat = 1.5; % ms - this is the interval between pulses in terms of the scanner variable
...
t_ramp = 0.25;  % ramp time in ms.

%Gz_ss = 0.6;  % slice select gradient in G/cm.
%Gz_ss = 0.4;  % slice select gradient in G/cm.
%Gz_ss = 0.01;  % slice select gradient in G/cm.


%   RF_spacing = 0.80 ; % time between RF pulses in ms.
% RF_spacing = 1.5 ; % time between RF pulses in ms.
%   RF_spacing = t_rfrepeat - t_Gz_ss - t_Gz_rephaser - 4*t_ramp;
RF_spacing = t_rfrepeat;

t_Gz_ss = 0.5 ; % ms
t_Gz_rephaser = t_rfrepeat - t_Gz_ss - 4*t_ramp ; % ms

% areas of gradients:  G/cm * ms
A_Gz_ss = Gz_ss * t_Gz_ss + Gz_ss * t_ramp;
...
myHanning0 = myHanning * flip_ang0 / (gam * dt);
...
for t = 1/dt + 1: RF_spacing/dt : 400/dt
    ...
    if cnt == 0
        % the first one is the "almost inversion"
        myHanning_x = myHanning0 .* FMx;
        myHanning_y = myHanning0 .* FMy;
        B1( t + t_ramp : ...
            t + t_ramp + length(myHanning) -1 ) = ...
            (myHanning_x  + i * myHanning_y) * doSign;
    end
    ...
    % slice select ramps:
    Gz(t : t + t_ramp-1) = linspace(0,Gz_ss, t_ramp);
    Gz( t + length(myHanning) + t_ramp : ...
        t + length(myHanning) + 2*t_ramp-1) = linspace(Gz_ss, 0, t_ramp);
    ...
    Gz( t + length(myHanning) + 2*t_ramp : ...
        t + length(myHanning) + 3*t_ramp -1 ) = ...
        linspace(0,-Gz_ss2, t_ramp);

    Gz( t + length(myHanning) + 3*t_ramp : ...
        t + length(myHanning) + 4*t_ramp -1 ) = ...
        linspace(-Gz_ss2, 0, t_ramp);
...
maxB1  = max(abs(B1))
mean_gradient = A_xtra /(t_rfrepeat)*1e3
mean_B1 = mean(myHanning) * (t_Gz_ss/t_rfrepeat)
finalMz = M(end,3)
display('----')
