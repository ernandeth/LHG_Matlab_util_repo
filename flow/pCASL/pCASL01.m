...
dt = 1e-3; % time step duration (1 us) expressed in millliseconds.  dt converts from samples to ms.
nstep = 500 / dt;  % 500 ms @  1 us steps.

t = linspace(0,500,nstep)'; % time vector in ms
flip_ang = flip_ang * pi /180;  % flip angle in radians
vel = velocity * 1e-6 ; % cm/s scaled to cm/us
t_ramp = t_ramp /dt;
...
% kludge: testing different pulses to see their effect 
% myHanning = make_gaussian(250, 20, 500)';
...
% extra phase to help with velocity profile
%
...
doSign = 1;
cnt = 0;
for t = t_ramp + 1: RF_spacing/dt : nstep - 10/dt
    ...
    FMx = cos(RF_dw  * 2*pi * [1:length(myHanning)]' * dt + RF_theta*cnt - RF_theta2 * cnt);
    FMy = sin(RF_dw  * 2*pi * [1:length(myHanning)]' * dt + RF_theta*cnt - RF_theta2 * cnt);
    cnt = cnt+1;
...
aq_phase = gambar *sum( Gz .* dz) * dt / cnt;
