...
Spower_MC = zeros(5,length(nlevels),NITER);
eff_MC = zeros(5,length(nlevels),NITER);
...
for nl=1:length(nlevels)
    ...
    for iter=1:NITER
        ...
        V = diag(1./sqrt(diag(V)))*V * noise_amp^2;
        ...
        for Dtype=1:5
            ...
            %         for c=1:3
            for c=1:2   % Do some clever indexing to work with presence
                % of intercept.
                effect_size = abs(B(end-(2-c)));
        ...
        Spower_MC (:,nl,iter) = Spower(:,2);
        eff_MC (:,nl,iter) = eff(:,2);
