...
close all
% Display flags:
global showSequence showMag slomo showBeff 
showSequence = 0;
showMag =1;
slomo = 0 ;
showBeff =1;
...
%seqparms(2) = 8 ; % N. of pulses in train ;
%seqparms(2) = 32 ; % N. of pulses in train ;
seqparms(3) = 0.2; % RF_duration ; % ms
seqparms(4) = 0.25 ;  % sweepWidth :  RF frequency sweep width (KHz)
...
flips = deg2rad(linspace(1,300,20));
flips = deg2rad(300);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ... or a continuous sech pulse
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
beta = 1;
k = 1;

% amplitude of the sech pulse
sech_amp = sech(beta * linspace(-pi , pi, hlen*Npulses));
...
sech_freq = -tanh(k*beta * linspace(-pi , pi, hlen*Npulses)); % works well
...
sech_tvec = RF_duration * Npulses * linspace(-0.5, 0.5, hlen*Npulses);
...
if showSequence
    ...
    subplot(411)
    plot(t, abs(B1)) ;    title('B1 magnitude')
    axis ([0 Npulses*modlength*dt 0 max(abs(B1)) ])
    
    subplot(412)
    plot(RF_dw) ;    title('Frequency')
    axis tight
    
    subplot(413)
    plot( t, angle(B1) );;    title('Phase')
...
filename = sprintf('myVSI_%d.bin',NPOINTS);
fp = fopen(filename,'wb','b');
fwrite(fp, tmp, 'int16');
fclose(fp);

% then use GE software to put in proper format
str = sprintf('!xlatebin -o myVSI_%d.rho myVSI_%d.bin', NPOINTS, NPOINTS);
eval(str)
...
% then use GE software to put in proper format
str = sprintf('!xlatebin -o myVSI_%d.theta myVSI_%d.bin', NPOINTS, NPOINTS);
eval(str)
...
% then use GE software to put in proper format
str = sprintf('!xlatebin -o myVSI_%d.grad myVSI_%d.bin', NPOINTS, NPOINTS);
eval(str)
