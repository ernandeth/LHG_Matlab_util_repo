...
% works well
seqparms(1) = deg2rad(90);  %flip_ang ;
seqparms(2) = 16 ; % N. of pulses in train ;
seqparms(3) = 0.5; % RF_duration ; % ms
seqparms(4) = 1 ;  % sweepWidth :  RF frequency sweep width (KHz)
seqparms(5) = 0 ;  % RF_phase correction
seqparms(6) = 0.1; % Gz_duration; % duration in ms.(includes both Gz lobes)
seqparms(7) = 0  ; % Gz_amp ; % Gauss / cm
...
% Things to try:
velocities = 5; % linspace(-50,50,50);
alphas=[];
sweep_range = [2:1:64];
starting_positions = [-10:10:10];
gmax_range = [0.5:0.5:3];
flips = deg2rad(linspace(10,200,50));
init_pos = [-15:5:15];
...
segment_mod = hanning (RF_duration/ dt);
hlen = length(segment_mod);

% Trying out other shapes of the RF envelope
% segment_mod = sinc(linspace(-pi, pi,hlen))';

% un-do the individual pulse modualtion
% so far, the modulation only screws things up.
%segment_mod(:) = 1;
segment_mod = segment_mod / sum(segment_mod*dt);
...
B1amp = B1area * segment_mod;  % units are Tesla
...
for n = 1: Npulses
    ...
    % version 1: constant frequency at each pulse
    RF_dw2 = RF_dw(n) * ones(size(segment_mod));  % kHz

    % version 2:  Here the frequency is also modulated throughout the individual 
    % pulse segments from 0 to RF_dw and back to 0
    % RF_dw2 = RF_dw(n) * segment_mod;  % kHz
    
    
    FM = exp(i *2*pi*  RF_dw2 .* linspace(0,RF_duration,length(segment_mod) )');
    ...
    % modulate pulses with Hanning window envelope (in Tesla)
    FMx = B1amp .* FMx;
    FMy = B1amp .* FMy;
    ...
    if showBeff
        ...
        B_eff2 = abs(B1seg) + i*RF_dw2 / gambar;
        
        % B_eff for the 'envelope' of the train in Tesla
        B_eff = weights(n) * max(segment_mod) + i * RF_dw/ gambar;
        
        prec_rate = gambar*abs(B_eff);   % in kHz
        prec_rate = prec_rate(1:end-1);
        tip_rate = diff(angle(B_eff)) / (length(Gzmodule)*dt);
...
if showSequence
    ...
    plot(t, imag(B1) ,'g')
    hold off
    axis ([0 length(B1module)*dt*Npulses -1e-5 1e-5])
