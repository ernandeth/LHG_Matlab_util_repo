...
Gvs =   1.250*1e-4;  % T/cm
Gvs =   3*1e-4;  % T/cm   % 16900 and 17268
...
vel_range = linspace(0, 30, 200) * 1e-3;
...
if batch_mode==0
    ...
    figure(5)
    % averaging over  velocities within voxel
    NetMz = cumsum(Mzfinal) ./ [1:length(Mzfinal)]';
    
    hold on
    plot(vel_range*1e3, NetMz)
    axis([min(vel_range)*1e3 max(vel_range)*1e3, -1 1])
    fatlines
    grid on
    xlabel('Velocity (cm/s)')
    ylabel('Net M_z')
    title('FT-VSI Velocity Profiles')
    %legend('Selective Case', 'Non-selective Case', 'Location', 'SouthEast')
    %print -dpng FTVS_profile_B0comp
