...
seqparms(6) = 4*4 ; %Gz_duration; % duration in ms.(includes all fours Gz lobes)
...
seqparms(6) = 20; % gz duraiton
seqparms(7) = 2.5; % Gzamp
seqparms(2) = 15; % Npulses
seqparms(1) = pi/seqparms(2); % flip angles

for p = 1

    inversion = zeros(size(accelerations));
    control = zeros(size(accelerations));
    
    for n = 1:length(accelerations)
        
        movparms(3) = accelerations(n);
        
        M = accel_sim(seqparms, movparms, 0);
        inversion(n) = M(end,3);
    end
    for n = 1:length(accelerations)
        ...
        Mc = accel_sim(seqparms, movparms, 1);
        control(n) = Mc(end,3);
    end
    ...
    plot(accelerations, control, 'g')
    plot(accelerations, control-inversion, 'r')
...
function M = accel_sim(seqparms, movparms, isControl);
%%
showSequence = 0;
showMag = 0;
slomo = 0;
