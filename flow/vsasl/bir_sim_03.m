...
Gz = [gap trap1 gap  gap gap (-trap1) gap  gap gap (-trap1) gap  gap gap  trap1 gap   gap];
...
tmp = tmp * dacmax / max(tmp);
tmp = 2*round(tmp/2);
filename = sprintf('BIR8_train.rho.txt');
fp=fopen(filename,'w');
...
tmp = tmp * dacmax / pi;
tmp = 2*round(tmp/2);
save BIR8_train.theta.txt tmp -ascii

filename = sprintf('BIR8_train.theta.txt');
fp=fopen(filename,'w');
for n=1:NPOINTS, fprintf(fp,'%d\n',tmp(n)); end
fclose(fp);
...
filename = sprintf('BIR8_train.grad.txt');
fp=fopen(filename,'w');
for n=1:NPOINTS, fprintf(fp,'%d\n',tmp(n)); end
fclose(fp);
