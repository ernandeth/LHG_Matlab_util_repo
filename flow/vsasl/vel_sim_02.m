...
seqparms(1) = deg2rad(5);  %flip_ang ;
seqparms(2) = 150 ; %Npulses ;
seqparms(3) = 0.5 ; %RF_duration ; % ms
seqparms(4) = 0 ;%RF_dw ;     % RF frequency offset
seqparms(5) = 0 ; %RF_phase ;  % RF phase
seqparms(6) = 0.2 ; %Gz_duration; % duration in ms.(includes all fours Gz lobes)
seqparms(7) = 0.1  ; %Gz_amp ; % Gauss / cm
seqparms(8) = 1.2;  % Gfac : fraction of refocus gradient (Gfac=1 mean balanced)
 
seqparms(1) = deg2rad(15);  %flip_ang ;
seqparms(2) = 150 ; %Npulses ;
seqparms(3) = 0.8 ; %RF_duration ; % ms
seqparms(6) = 0.2 ; %Gz_duration; % duration in ms.(includes all fours Gz lobes)
seqparms(7) = 0.5  ; %Gz_amp ; % Gauss / cm
seqparms(8) = 0.8;  % Gfac : fraction of refocus gradient (Gfac=1 mean balanced)



velocities = [-150:10:150];
...
% select several starting positions
for pos = [-5];
    pos
...
zpos_target = vel_target*ttmp + accel_target*(ttmp.^2);
