...
Gseg = 3e-3;  % seconds
Gseg = 6e-3;  % seconds : LHG 4/14/22 - use for diffusion
NGpoints = round(Gseg/sampTime);

trap1 = zeros(1, NGpoints);
gspace = 80 ;  % a little gap between the RF and the VENC to mititgate eddy currrents

trap1(1 : NGpoints*1/3) = [0: NGpoints*1/3-1];
trap1(NGpoints*1/3+1   : NGpoints*2/3 ) = [NGpoints*1/3-1: -1:0];
trap1 = trap1 /max(abs(trap1))*Gmax;
...
Gz = [ggap trap1 ggap ggap (-trap1) ggap ggap (-trap1) ggap ggap trap1 ggap];
