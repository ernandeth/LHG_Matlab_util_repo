function [dict, diff,parms] = gen_dictionary_140712 (timing_parms,doSub)
...
% cbvavals = linspace(0.01, 0.05, 3);
cbvavals = 0.02;

transvals = linspace(0.2, 2.5, 20);
%transvals = 0.8;
...
%     matlabpool close 
    ...
    entry = gen_signals_140707(parms(1) , timing_parms, dofigs, doSub);
    Nobs = length(entry);
    Ncombinations = n-1;
    dict = zeros(Ncombinations, Nobs);
    diff = zeros(Ncombinations, Nobs);
    parms = parms(1:Ncombinations);
    
    for n=1:Ncombinations;
        % fprintf('\rGenerating entry .... %d    of  %d   ',n, Ncombinations);
        entry = gen_signals_140707(parms(n) , timing_parms, dofigs, doSub);
        dict(n,:) = entry;
        entry = gen_signals_140707p(parms(n) , timing_parms, dofigs, doSub);
        diff(n,:)=entry;
        parms(n);
    end
    ...
    save dictionary dict diff parms ;
