...
maxTR = 100/length(x);  % keep this at 20 seconds, so 0.5 seconds per TR
...
% linear increase in t_tag didn't seem to improve things much
% timing_parms.t_tag = linspace(0.08, 0.4, N);
% timing_parms.t_tag =  0.4 * ones(size(tvec));
% load a pre-shuffled version of the linear schedule
...
% disregard early part of dictionary.
dict = dict(5:end, :);
