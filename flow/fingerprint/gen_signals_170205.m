function  obs = gen_signals_160426(parms, timing_parms, dofigs,doSub)
...
for n=2:length(timevec)
    ...
    dM = (mtis0 - M(n-1))*r1tis  - kfor(n-1)*M(n-1) + f * inMa(n-1)/lambda  - f * M(n-1)/lambda;
        
    M(n) = M(n-1) + dM*dt;
    
    % # 2/6/17 new stuff:
    dMa = (1-Ma2(n-1))*r1a + Ma(n) - Ma2(n-1);
    Ma2(n) = Ma2(n-1) + dMa*dt;
    ...
    if aqfun(n-1)==1
        ...
        obs(aq) = ((1-cbva)*M(n-1) + cbva*Ma2(n-1)) * sin(flip);
