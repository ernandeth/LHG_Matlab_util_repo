...
% 1) some silent labeling periods
% 2) It varies the tagging durations
% 3) It turns on and off the post-inversion delays
...
%}
%%
TR =        sort(repmat( linspace(2.2, 4, 20) , [1,4]));

label_dur = sort(repmat( linspace(0.2, 1.8, 20) , [1,4])); %  ]TR - 0.600 - 0.300;
TR(:) = 4;

PID = PID*ones(size(label_dur));
PID(3:8:end) = 1.6;
PID(4:8:end) = 1.6;
%PID = sort(repmat( linspace(0.2, 2, 10) , [1,8]));
