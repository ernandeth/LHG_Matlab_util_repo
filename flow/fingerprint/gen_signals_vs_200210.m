function  obs = gen_signals_vs_200210(parms, aq_parms, dofigs,doSub)
...
% error
aqfun = [aqfun;zeros(ceil(ArtSup_delay(end)/dt),1)];
...
for n = nbat+1:length(timevec)
    ...
    % selective pulse
    if vsfun(n-1) == 1
        Ma(n) = Ma(n)* (1-alpha_ai);  %%% LHG 10.22.
