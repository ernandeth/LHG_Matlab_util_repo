...
for Ncycles = Ncycle_vals
    ...
    for maxtime = maxTime_vals
        ...
        %{
        % Anish schedule:
        tmp = load('timing_files_201/del1.txt');
        timing_parms.del1 = tmp(:);
        tmp = load('timing_files_201/del2s.txt');
        timing_parms.del2 = tmp(:);
        %}
        ...
        timing_parms.RO_type                ='FSE';

        % LHG 3/15/22
        timing_parms.del2              = 0.75 + (maxtime/2)*(1+chirp(linspace(0,1, Nframes), 0, 1, Ncycles))';
        timing_parms.del3              = 0.05 + (maxtime/4)*(1+chirp(linspace( 1,0, Nframes), 0, 1,1.25*Ncycles))';
        %timing_parms.del1              = 0.5*ones(Nframes,1) ;
        timing_parms.del1              = 1.8 -timing_parms.del2 - timing_parms.del3;
        ...
        best_NRMS = 0;
        for t=1:1000  % random orders : choose the best_NRMS
...
% 
        ...
        seqDuration = sum(timing_parms.del2)+ ...
        ...
        if DEBUG
            figure(1)
            subplot(211)
            plot(timing_parms.del2); title('t delay')
            axis tight
            
            subplot(212)
            plot(timing_parms.del3); title('AS delay')
            axis tight
...
figure
intervals = timing_parms.del2+timing_parms.del3;
plot(intervals)
title('AStime + t delay')

make_schedule_files(...
    best_timing_parms, ...
    '/home/hernan/matlab/Matlab_util_repo/asl_mrf_nets/chirp_vssx2_schedule_250_loSAR');
