function [dict, parms] = gen_flex_dictionary_150207 (timing_parms, fixed_parms)
% function [dict, parms] = gen_flex_dictionary_150207 (timing_parms, [fixed_parms])
...
transvals = linspace(0.5, 3, 20);
kforvals = linspace(0,0.1,20);
r1vals = 1./linspace(0.5,3,20);
betavals = linspace(20,60,20) * pi/180;
...
if nargin==2
    ...
    if isfield(fixed_parms, 'transit') ,  transvals = fixed_parms.transit;  end
    
    if isfield(fixed_parms, 'kfor'),  kforvals = fixed_parms.kfor;  end
    
    if isfield(fixed_parms, 'r1tis'),  r1vals = fixed_parms.r1tis;  end
    
    if isfield(fixed_parms, 'beta'),  betavals = fixed_parms.beta;  end
    
    if isfield(fixed_parms, 'Disp'),  Dispvals = fixed_parms.Disp;  end
    
end

Ncombinations = length(Dispvals) * length(betavals) * length(r1vals) * ...
    length(kforvals) * length(transvals) * length(cbvavals) * length(fvals) * length(mtis0vals) ;
