function  obs = gen_signals_vs_220421(parms, aq_parms, dofigs,doSub, dt)
...
bolus_length_aq = round(aqwindow*100)/100; %0.5; % bolus created by aquisition
%(assume a labeling pulse is always applied after readout, to reset the arterial magnetication)
BN = round(bolus_length/dt);
BN_aq = round(bolus_length_aq/dt);
...
% error
aqfun = [aqfun; zeros(round(ArtSup_delay(end)/dt),1)];
