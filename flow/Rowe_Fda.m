...
% 2 numerator and n-2 denominator degrees of freedom
...
for count=1:p
    % real and imaginary come out as separate comlumns (see the inner loop)
    sigma2hat(count,1) = ...
        (Ycomp(:,count) - X*BhatRI(:,count))'*(Ycomp(:,count) - X*BhatRI(:,count))...
        /(2*n); % ' does Hermetian
    
    if sigma2hat(count,1)~=0
        % F score is variance of the contrast  parameter estimates
        % over residual variance.  see stat4GLMPrint.pdf slide 29 and 33
        fstatda(count,1) = ...
            C*[real( BhatRI(:,count)) , imag(BhatRI(:,count)) ]*...
            invCWCt*[real(BhatRI(:,count)),imag(BhatRI(:,count))]'*C'...
            / sigma2hat(count,1);
    end
end
