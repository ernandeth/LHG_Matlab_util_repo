...
% Typical values:
 Ttag =0.8;  % seconds
 Ttag =3.5  % seconds   (Difference between turbo and regular)
 del = 0.02;	 %seconds
 crushers=1;
 R1t = 1/1.2;    % 1/sec.
 R1a = 1/1.6;    % 1/sec
...
signal = kinetix_lsq( f_est, t, parms); 
...
% Adding the additional mean parameter to the initial guess 
est0(:) = f(1)-mean(f);
est0(end) = mean(f)*0.7;
%%%%
...
% put the baseline back in the flow estimate
