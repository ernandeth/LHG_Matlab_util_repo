function result = kinetix_ss_lsq(estimates,time,parms, data)
%function result = kinetix_ss_lsq(perfusion_estimate,time, parms, [data] )
% 
% Lax-Wendroff simulation of flow experiment:
% for use with lsqnonlin
%
% uses a constant perfusion estimate value to estimate the signal.
%
...
if ~isempty(find(V > dx/dt))
	fprintf('\r Warning ... V is too high' )
