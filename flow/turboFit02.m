function [f, Ttrans]=turboFit02 ( tr_vec, Thres, dist, alpha, root)
%function [f, Ttrans]=turboFit02 ( tr_vec, Thres, dist, alpha, root)
%
% this version takes into account the deispersion of the arterial tag
%
...
for pix=1:size(data,2)
	...
	if Verbose==0 & rem(pix,1000)== 0
        ...
        if Verbose==1
	    fprintf('\rfitting ...pix %d of %d ...sl=%d - f^= %f Ttrans^= %f beta^= %f - guessed: %f %f %f',...
                pix , size(data,2),sl, f(pix), Ttrans(pix), Beta(pix), f_guess, Ttrans_guess, Beta_guess);
