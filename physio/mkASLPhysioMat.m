function PhysioMat = mkASLPhysioMat(physfile, sampTime, disdaq, Nslices, TR, Ttag, slTiming)
% function PhysioMat = mkASLPhysioMat(physfile, sampTime, disdaq, Nslices, TR, Ttag, [slTiming])
% 
...
% shifting the model for each slice
%
...
PhysioMat = zeros(Nslices, Nimgs, 13);
aslmod = ones(Nimgs,1);
aslmod(1:2:end)=-1;
...
for slnum=0:Nslices -1
    ...
    DM = [ones(Nimgs,1) aslmod polynomial cph rph   ];
