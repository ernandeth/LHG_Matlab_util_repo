function myCompCor(rootname, DesMat, TR, A)
...
if isfield(h,'xdim')
    isAVW = 1;
    fprintf('your data are in AVW format, but Im going to give you NIFTI');
    h = avw2nii_hdr(h);
end
...
% 1. remove the design matrix from the data as much as possible
fprintf('\nQuick detrend ...');
Y1 = mydetrend(Y);
fprintf('\nRegressing out the Design Matrix ...');
Y1 = Y1 - DesMat*pinv(DesMat)*Y1;

% 2. Anatomical Noise ROI

% anoiseROI(data, header, rawDM);

% 3. Temporal Noise ROI : identify the highest variance voxels
...
Yclean = CompCor_PCA_wash(Y, NewData, DesMat, TR,[]);
fprintf('\nWriting out clean data (CompCor_residuals.nii)');
write_nii('CompCor_residuals.nii', Yclean, h, 0);

% 5. diagnostics : how much is the variance reduced?
...
%%
function Yclean = CompCor_PCA_wash(Y0,  Yred, DesMat, TR , Ncomponents)

% function CompCor_PCA_wash(raw_data, reduced_data, DesMat, TR [, Ncomponents])
%
...
%
% INPUTS:
%   Y0:  raw time series data:  e.g. 'ravol'
%
%   Y:  time series of data with highest variance
%
...
BadRegs = [1:10];
X = Tcomponents(:, BadRegs);
X = [ones(length(X), 1) X];
figure
imagesc(X); colormap('gray'); title('Removing these components'); drawnow

% estimate paramaters for components
betahat = pinv(X)*Y0;

% remove components - except for the first one (intercept)
