...
if isstr(root)
  for n=1:Ncon
    ...
    write_hdr( sprintf('CompCor_Tmap_%04d.hdr', n), outh);
    write_img_data( sprintf('CompCor_Tmap_%04d.img',n), tmap(n,:), outh);

    outh.glmax = max(vCon(n,:));
    outh.glmin = min(vCon(n,:));
    write_hdr( sprintf('CompCor_ConVar_%04d.hdr', n), outh);
    write_img_data( sprintf('CompCor_ConVar_%04d.img',n), vCon(n,:), outh);

    outh.glmax = max(zmap(n,:));
    outh.glmin = min(zmap(n,:));
    write_hdr( sprintf('CompCor_Zmap_%04d.hdr',n), outh);
    write_img_data( sprintf('CompCor_Zmap_%04d.img',n), zmap(n,:), outh);
    warning on
  end
end

fprintf('\nSaving Intermediate data to CompCor_spmJr.mat file ...');
save CompCor_spmJr beta_est contrast DesMat vCon df zmap tmap
