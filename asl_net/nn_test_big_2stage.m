...
save training.mat trainingData trainingParms
%
%
...
Mynet=[inputLayer; f0; r1; f2; r1; f3; r1; f4; r6]
...
%
%%  Now do a second network to estimate perfusion only:
fprintf('\nGenerating Network for perfusion only ....');
% We will concatenate the parameters estimates, except for perfusion,
% at the end of the training time series - as it they were part of the time series.
% The new trainingData time series are Npoints long 
% plus the 5 paramaters estimated previously
% recall that the trainingParms are already scaled!
%{
Npoints2 = Npoints+5;

Npoints2 = Npoints;

% First undo the scaling of the training paramaters for this
%tmp = trainingParms ./ repmat(scale_vector, Ncombinations,1);

%trainingData2 = squeeze(trainingData);
%trainingData2 = [trainingData2 ; tmp(:, 2:end)'];
%trainingData2 = reshape(trainingData2, Npoints2, 1,1,Ncombinations);

% The new training Parms is just the perfusion (1st column) still scaled
trainingParms2 = trainingParms(:,1);

% now make a new network
inputLayer = imageInputLayer([Npoints2 1]);
r1 = reluLayer();
f0 = fullyConnectedLayer(150);
f1 = fullyConnectedLayer(75);
f2 = fullyConnectedLayer(25);
f3 = fullyConnectedLayer(12);
f4 = fullyConnectedLayer(1);
r6 = regressionLayer;

Mynet_f=[inputLayer; f0; r1; f2; r1; f3; r1; f4; r1; r6]

Mynet_f=[inputLayer; f1; r1; f3; r1; f4; r1; r6]
...
save NN_stuff_f Mynet_f timing_parms Npoints2 scale_vector

tic
fprintf('\nTraining Network ....');

% train network with output from previous network
% Mynet_f = trainNetwork(trainingData2, trainingParms2, Mynet_f, opts )

% train network with same data as previous network but get perfusion only
Mynet_f = trainNetwork(trainingData, trainingParms2, Mynet_f, opts )

toc
%

%%
fprintf('\nTesting Network #1 with new synthetic data (linear)....');
...
fprintf('\n Testing network 1 with random Parameters ....');
...
%% Now test the second network with the previous set of random parameters

fprintf('\n Testing network 2 (perfusion only) with random Parameters ....');

% I will replace the previous estimates of flow only
...
for n=1:Ntests
    ...
    % concatenate the previous parameter estimates into the time series
    % but first undo the scaling of the training paramaters for this
    tmp = truth ./ scale_vector;
    test_data = [test_data; tmp(2:end)'];
    
    test_data = reshape(test_data, Npoints2, 1,1);
    prediction2 = predict(Mynet_f, test_data);
    
    % undo the scaling:
    prediction2 = prediction2 / scale_vector(1);
    
    estimates2(n,1) = prediction2;
    
    % calc error (MSE)
    mse(n) = mean(sqrt((truth  - prediction2).^2) ./ truth);
...
for n=1:6
    subplot(3,2,n)
    plot(testparms(:,n), estimates2(:,n), 'x')
    axis([min(testparms(:,n))  max(testparms(:,n)) min(testparms(:,n)) max(testparms(:,n))])
    title( (mytitles(n)))
    axis square
end

set(gcf,'Name', 'Results from Second Network')
