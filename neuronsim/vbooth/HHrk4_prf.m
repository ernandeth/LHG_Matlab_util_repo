...
%cf = 800;  % carrier frequency in kHz
prf = .0001;
%USflag = 1;
...
uspw = heaviside(tt-uston).*sin(2*pi*cf.*(tt-uston)) .*(1 + square(2*pi*prf.*tt))/2;

for i=1:numsteps
    

    
    k1 = dt*hheqs(t, v, USflag,uspw(i), mods);
    k2 = dt*hheqs(t + halfdt, v+0.5*k1, USflag, uspw(i), mods);
    k3 = dt*hheqs(t + halfdt, v+0.5*k2, USflag, uspw(i), mods);
    k4 = dt*hheqs(t + dt, v+k3, USflag, uspw(i), mods);
