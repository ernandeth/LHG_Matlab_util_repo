...
if ~isvar('Gm'), printm 'setup Gnufft object'
	ig = image_geom('nx', 64, 'ny', 64, 'dx', 0.1, 'offsets', 'dsp');
% 	ig.mask = ig.circ(1+ig.nx/2, 1+ig.ny/2) > 0;
	N = ig.dim;
	J = [6 6];

	[kspace omega wi] = mri_trajectory('spiral1', {}, N, ig.fov, {'voronoi'});
...
prompt, clear kspace nufft_args
end
...
if 0
	...
	yd_g = griddata(omega(:,1), omega(:,2), double(yi), oo1, oo2, 'linear');
	yd_g(isnan(yd_g)) = 0;
