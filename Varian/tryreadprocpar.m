% tryreadprocpar.m
% reads Vnmrj produced procpar parameter files into a matlab structure.
% procpar = tryreadprocpar(procdir,verbose)
% Inputs: directory: String. Location of desired procpar file.
%         verbose: (optional) 0 is default.
% Outputs: procpar: Matlab structure containing procpar variables.
% 
%      usage: readprocpar.m()
%         by: justin gardner
%       date: 07/03/03
...
%
function procpar = tryreadprocpar(procdir,verbose)
...
while (line ~= -1) 
  ...
  token = strtok(line);
%    if strcmp(token,'spiral_in')
%        dbstop in tryreadprocpar at 55
%    end
  ...
  if (~((token(1) >= '0') & (token(1) <= '9')) & (token(1) ~= '"'))
	...
	if (isempty(token) | (token(1) ~= '"'))
	  ...
	  str(findstr(str,'''')) = '"';
      if numel(strfind(fieldname,'$'))<1;    % Get rid of the non-recognized expression $
