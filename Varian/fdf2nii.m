function [data h ] = fdf2nii(pathname, doJPG , scale)
...
while  (counter_line < 100)
	...
	if strmatch('float  array_dim ', line)
   	...
   	if strmatch('float  array_index ', line)
		pos = find(line == '=');
		dim(4) = str2num(line(pos+1:end)) +1;
	end
...
% the dim(4) doesn't always work.  try this:
dim(4)=length(dir([pathname 'slice001*echo001.fdf']));
