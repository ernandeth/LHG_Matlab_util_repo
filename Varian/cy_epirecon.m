...
%   complex or SDT(abs)
%
% sample: [raw, lro, lpe, thk, total_tr, te, ti, ref_pos, ss, sli_val] = epirecon(filename);
...
if( exist([ '~/matlab/recon/tablib/' petable ], 'file') )
    pathpe = [ '~/matlab/recon/tablib/' petable ];
elseif( exist( [ './' petable ], 'file') )
    pathpe = [ './' petable ];
else
    error('table: %s does not exist in ~/matlab/recon/tablib/ or ./', petable)
end
