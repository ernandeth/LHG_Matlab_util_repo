...
if (slicenum==Nslices)
    ...
    if DO3D && DORECON
        ...
        tmp3drec = tmp3drec - mean(tmp3drec(:));
    ...
    if DEBUG~=2
        ...
        figure(1);
        lightbox(reshape(volumes(vnum,:),xres,xres,Nslices),[],NROWS );
    ...
    if ~mod(vnum-3,6) & vnum>8
        ...
        if ~DOCORRELATION && ~DOREGRESSION
            ax2 = figure(3);
            lightbox(reshape(cumasl, xres, xres, Nslices),[-250 250],NROWS);
    ...
    if (vnum>2 && vnum<Nframes)
        ...
        asl = ASLORDER*asl*0.5*(-1)^vnum;
        
        asl = asl.*slmask;
        if DEBUG~=2
            
            ax1 = figure(2);
            lightbox(reshape(asl, xres, xres, Nslices),[-50 400],NROWS);
        ...
        if DEBUG~=2
            ...
            %junkreg = mean_asl_course -  mean(mean_asl_course);
