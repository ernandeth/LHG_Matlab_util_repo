hrfunction outdata = smoothdata2(data,  TR, locutoff, hicutoff, N_coeffs,  verbose)
% function outdata = smoothdata2(data,  TR, low_cutoff, high_cutoff, N_coeffs [,verbose])
%
...
% low-cutoff  -  the desired cut-off frequency in Hz.
% high-cutoff  -  the desired cut-off frequency in Hz.
...
%
    ...
    hicutoff = hicutoff  / nyquist;
    locutoff = locutoff  / nyquist;
    
    b = remez(N_coeffs, [0 locutoff-0.005  locutoff+0.005 hicutoff-0.005  hicutoff+0.005  1], [0 0 1  1 0 0]);
...
%    data = data(:,2);
    ...
    subplot(2,1,1) , hold on,plot(time, (data/mean(data)))
...
%   
    ...
    plot(time, (outdata/mean(outdata)), 'r'), 
    title ('Time Domain')
    legend('Before', 'After')
 
    subplot(2,1,2), hold on, 
    plot(f, abs(outfdata), 'r'), 
