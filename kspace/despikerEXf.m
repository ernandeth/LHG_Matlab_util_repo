function indices=despikerEX(pfilename ,fastrecflag, showData)
% Usage ... indices = despikerEXf(pfile,fastrecflag, showData)
%
% this program removes spikes in the kspace data by identifying 
% when and where you get a kspace vaulue that is much larger than 
% the standard deviation at that k-space location over the time series.
%
% THIS VERSION WORKS IN THE FREQUENCY DOMAIN OF TIME
%
...
for sl=1:nslices
    ...
    for col=1:size(out,2)
        ...
        badrows = find(abs(out(:,col)) > ...
            (abs(means(col)) + 3*abs(stds2(col))) );
        indices=[indices ; badrows];
        fprintf('\rCorrecting frequency %d of %d slice %d -first pass',col,size(out,2),sl);
    ...
    for col=1:size(out,2)
        ...
        badrows = find(abs(out(:,col)) > ...
            (abs(means(col)) + 3*abs(stds2(col))) );
        fprintf('\rCorrecting Frequency %d of %d slice %d - second pass',col,size(out,2),sl);
        for r=1:length(badrows)
            row=badrows(r);
            out(row,col) = means(col);
        end
...
if 0  % this doesn't work so well.... 
    % filter 3 - look at derivatives
    diffs = abs(diff(out,1,1));
   
    for row =1:size(diffs,1)
        badcols = find(diffs(row,:) > mean(diffs(row,:)) + 2*std(diffs(row,:)) );
        rowmean = mean(out(row,:));
        fprintf('\rcorrecting frequency %d of %d slice %d - third pass',col,size(out,2),sl);
        out(row,badcols) = rowmean;
    end
    fprintf('\n'); 
