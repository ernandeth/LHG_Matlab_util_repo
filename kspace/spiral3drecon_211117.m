function [tmpk, im_all] = spiral3drecon_211031(myPfile,doRots, Ndel,  doNUFFT)
...
if doRots  % SERIOS readout: rotating spiral pancakes about x axis
    if useKrot  
        ...
        for n=0:nleaves-1
            for m=0:nslices-1
                angle_list(count,1) = deg2rad(137.5)*m + (pi/nslices)*n/nleaves;  % slice rotations
                angle_list(count,2) = zrot_angle*n;      % leave rotations
                count = count +1;
            end
        end
    end
...
%   for c = 1:ncoils
        ...
        if doPhaseDetrend
            ...
            %{
            subplot(311) , plot(squeeze(angle(b(end,:,1)))); title('phase across shots')
            subplot(312) , plot(squeeze(angle(b(end, 1,:)))); title('phase across slices')
            subplot(313) , plot(unwrap(angle(squeeze(b(end/2-10:end/2+10, 1,1))))); title('phase in the center segment')
            drawnow
            pause
            %}
        end
...
if nframes>3
    h.dim(5) = 1;
    write_nii('timeseries_var.nii', var(imseries(:,3:end),[],2), h,0);
end
return
%%
...
%{
slope = (p(end/2+10)-p(end/2-10)) / 20;
