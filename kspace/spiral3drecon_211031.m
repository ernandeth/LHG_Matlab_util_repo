...
%    for c = 1:ncoils
        ...
        % gkdata = interp3(ks(:,1), ks(:,2), ks(:,3), tmpk(:), kx, ky, kz, 'spline');
...
if nframes>3
    h.dim(5) = 1;
    write_nii('timeseries_var.nii', log(var(imseries(:,3:end),[],2)), h,0);
end
return
%%
...
% use the center points to establish a line slope
slope = (p(end/2+10)-p(end/2-10)) / 20;
lintrend = p(end/2) + [0:length(p)-1]*slope;
p2 = p -lintrend' ;

%{
% linear detrend the whole echo
p2 = detrend(p,1) ;
p2 = p2-p2(end/2);
%}
%{
