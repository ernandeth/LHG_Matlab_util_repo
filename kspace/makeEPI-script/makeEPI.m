function [gx,gy] = makeEPI(fov, npix, etl, prefix, issoft)
% function [gx,gy] = makeEPI(fov, npix, etl, prefix, [issoft])
%
...
%  numshots = 3;
%  fov = 24;
%  makeEPI(fox,etl*numshots,etl,'myepireadout');
%
% $Id: makeEPI.m,v 1.9 2010/04/05 18:45:56 jfnielse Exp $
...
if issoft
	mxg = 2.6;
	mxs = 0.75*15.0;                 % make gradients quieter
else
	mxg = 4.0;
	mxs = 15.0;
end
...
area = sum(gxro)*4e-6;
gxprew = -trapwave(area/2, 4e-6, mxg/sqrt(2), mxs/sqrt(2)*1000);       % x and y gradients playing after daq
gxprew = [zeros(1,mod(length(gxprew)+length(ramp),2)) gxprew];

% partial dephaser (one cycle of phase across each voxel)
gxpd = -trapwave(areapd/4, 4e-6, mxg/sqrt(3), mxs/sqrt(3)*1000);       % y and z phase-encode blips playing at the same time 
gxpd = [zeros(1,mod(length(gxpd),2)) gxpd];

% phase-encode trapezoids before/after gx
gyprew = -trapwave(areapd/2, 4e-6, mxg/sqrt(3), mxs/sqrt(3)*1000);
