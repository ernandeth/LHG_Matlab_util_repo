...
%
% OUTPUT:
% this new version returns a singel network for all the coils
%
%
...
for coilnum=1:Ncoils
    ...
    subplot(2,1,2)
    lightbox(abs(fftshift(ifftn(tmp))));
...
for p = 1:Npatches
    ...
    % and imag for the output of the neural net.
    ...
    % the first half of the output rows will be REAL and the second IMAG.
    % New Dimension: (Nnbrs*Ncoils*2) x Ncoils ...
    W2 = [real(W); imag(W)];


    % W2 is the solution to the whole system of equations that we constructed,
    % so each of these equations can be used as a training example
    % make a 2D matrix for each of the equations (targets)
    %  -->    (Nnbrs*Ncoils*2) x Ncoils  x Nequations
    Wblock= repmat(W2, 1,1,Nequations);


    % We'll now build a network that will learn the weights for a specific
    % set of input locations (relative to the target)
    % the input will be the 3D coordinates of the neighbors (relative to
    % the target) - [Nnbrs x 3]
    % The output of the network will be the weights corresponding
    % to each ot the neighbor locations and across coils
    % - (Nnbrs*Ncoils*2) x Ncoils
    %
    % Note that in previous version we made a network for each coil to make the
    % problem smaller.  so the output  of the networks will be [Nnbrs*Ncoils*2 x 1]
...
%}
%-------------------------------------------
% Now trying to make it wider ... 10/22/2023
% This one tries to estimate allt the coils with a single massive network.
%%
%{
GWnet = [
...
% added Layer on 10/30/23
fullyConnectedLayer(Ncoils*Nnbrs*Ncoils);
batchNormalizationLayer;
reluLayer;

fullyConnectedLayer([Nnbrs*Ncoils*2*Ncoils]);
regressionLayer
]
%}
%---- try a deeper network to catch the exponentials (non-linearities)
GWnet = [
...
fullyConnectedLayer(Ncoils*Nnbrs*Ncoils*10);
batchNormalizationLayer;
reluLayer;
...
% gpuTrainDataIn = gpuArray(TrainDataIn);
% TrainDataOut = gpuArray(TrainDataOut);

% Now train this network for all the coils at once
%  TDO = squeeze(TrainDataOut(:,coilnum,:))';
TDO = reshape(TrainDataOut, Nnbrs*Ncoils*2*Ncoils, Ntrain)';
%gpuTrainDataIn = gpuArray(TrainDataIn);
...
Truth = squeeze(TDO(justafew,:));

est = zeros(size(Truth));

p=1;
for n =1:length(justafew)
    tmp = predict(allCoilNets , TestInput(:,:,:,n));
...
title(['Correlation: all coils combined'])
drawnow

figure
hist(TestInput(:),100)
title('Distribution of Test input values to network')
%}
%%
%fprintf('\nSaving networks ...')
