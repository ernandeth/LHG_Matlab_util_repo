...
dt = 4e-6 ; % s
Kmax = 1/(xres)/2;
deltaK = 1/FOV/2;        % resolution of k-space (separation between turns)
...
% Now do the gridding  reconstruction
% a k-space grid
...
% Now do a NUFFT recon
figure(2); 

ksxyz = [kxs(:) kys(:) kzs(:)];

[nufft_img, Gob] = nufft3d(ksxyz , signal, [24, 24, 24]/100, [64 64 64]);

subplot(224)
lightbox(abs(nufft_img)); title('The NUFFT CP reconned image')
