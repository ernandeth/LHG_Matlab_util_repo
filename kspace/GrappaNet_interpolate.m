...
fprintf('\nDone with griddata  ... now  using Neural Net',  coilnum)
...
for coilnum=1:Ncoils
    ...
    for p =  1:Npix
        ...
        if length(inds) < Nnbrs
            ...
            if mod(p,1e3)==0
                ...
                drawnow
                %
                fprintf('\nProgress ... Just Interpolated Kvoxel %d of %d \n', p, Npix);
                whos W patch
                toc
            end
            %}
    ...
    figure
    subplot(221)
    plot(abs(tmp(:)), abs(tmp2(:)),'o');
    rho = corrcoef(abs(tmp(:)), abs(tmp2(:)));
    title(sprintf('k space for coil %d, rho = %f', coilnum, rho(1,2)));
