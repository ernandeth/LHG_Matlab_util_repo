...
% Usage ... indices = despikerEX(pfile,fastrecflag, showData)
%
...
for sl=1:nslices
    ...
    for col=1:size(out,2)
        ...
        fprintf('\rCorrecting point %d of %d slice %d -first pass',col,size(out,2),sl);
    ...
    for col=1:size(out,2)
        ...
        badrows = find(abs(out(:,col)) > ...
            (abs(means(col)) + 3*abs(stds2(col))) );
        fprintf('\rCorrecting point %d of %d slice %d -second pass',col,size(out,2),sl);
        for r=1:length(badrows)
            row=badrows(r);
            out(row,col) = means(col);
        end
