function hdr = read_gehdr(id,offset)
...
%         >> hdr = read_gehdr(id);
%
%  note:  Raw file must have RDB_RDBM_REVISION = 20.006F

%  This script was generated from GE source using 'gehdr2matlab20' modified from
%  'gehdr2matlab' written by DB Clayton

% $Id: read_shifted_gehdr.m 1279 2014-03-24 20:06:25Z klitinas $
% $HeadURL: svn+ssh://klitinas@anger.engin.umich.edu/svn/matlab/img/trunk/read_shifted_gehdr.m $


fseek(id, 0, -1);  % go to start of raw data file
hdr.rdb.rdbm_rev = fread(id, 1, 'float');  
hdr.rdb.run_int = fread(id, 1, 'int');  % Rdy pkt Run Number 
...
hdr.rdb.exec_ctrl = fread(id, 1, 'ushort');  % rhexecctrl bit mask 15 bits 
...
hdr.rdb.rc_zres = fread(id, 1, 'int');  % power of 2 > rhnslices 0 - 128 
hdr.rdb.raw_pass_size_deprecated = fread(id, 1, 'int');  
hdr.rdb.sspsave_deprecated = fread(id, 1, 'int');  
hdr.rdb.udasave_deprecated = fread(id, 1, 'int');  
...
hdr.rdb.v_type = fread(id, 1, 'int');  % rhvtype bit mask 31 bits 
...
hdr.rdb.ps_command = fread(id, 1, 'int');  % PS internal use only 
hdr.rdb.ps_mps_r1 = fread(id, 1, 'int');  % PS MPS R1 setting 1 - 7 
hdr.rdb.ps_mps_r2 = fread(id, 1, 'int');  % PS MPS R2 setting 1 - 30 
hdr.rdb.ps_mps_tg = fread(id, 1, 'int');  % PS MPS Transmit gain setting 0 - 200
hdr.rdb.ps_mps_freq = fread(id, 1, 'uint');  % PS MPS Center frequency hz +-3.40282e38 
hdr.rdb.ps_aps_r1 = fread(id, 1, 'int');  % PS APS R1 setting 1 - 7 
hdr.rdb.ps_aps_r2 = fread(id, 1, 'int');  % PS APS R2 setting 1 - 30 
hdr.rdb.ps_aps_tg = fread(id, 1, 'int');  % PS APS Transmit gain setting 0 - 200
hdr.rdb.ps_aps_freq = fread(id, 1, 'uint');  % PS APS Center frequency hz +-3.40282e38 
hdr.rdb.ps_scalei = fread(id, 1, 'float');  % PS rational scaling +-3.40282e38 
hdr.rdb.ps_scaleq = fread(id, 1, 'float');  % PS unused 
hdr.rdb.ps_snr_warning = fread(id, 1, 'int');  % PS noise test 0=16 1=32 bits 0, 1 
hdr.rdb.ps_aps_or_mps = fread(id, 1, 'int');  % PS prescan order logic 0 - 5 
hdr.rdb.ps_mps_bitmap = fread(id, 1, 'int');  % PS bit mask 4 bits
hdr.rdb.ps_powerspec = fread(id, 256, '*char')';  % PS 
hdr.rdb.ps_filler1 = fread(id, 1, 'int');  % PS filler 
hdr.rdb.ps_filler2 = fread(id, 1, 'int');  % PS filler 
...
hdr.rdb.data_collect_type1 = fread(id, 1, 'int');  % rh???? bit mask 31 bits 
...
hdr.rdb.vrgf = fread(id, 1, 'int');  % control word for VRG filter 
hdr.rdb.vrgfxres = fread(id, 1, 'int');  % control word for VRGF final x resolution 
hdr.rdb.bp_corr = fread(id, 1, 'int');  % control word for bandpass asymmetry 
hdr.rdb.recv_freq_s = fread(id, 1, 'float');  % starting frequency (+62.5) 
hdr.rdb.recv_freq_e = fread(id, 1, 'float');  % ending frequency (-62.5) 
hdr.rdb.hniter = fread(id, 1, 'int');  % Selects the number of (continued...)
hdr.rdb.fast_rec = fread(id, 1, 'int');  % Added for homodyne II, tells if (continued...)
hdr.rdb.refframes = fread(id, 1, 'int');  % total # of frames for ref scan 
hdr.rdb.refframep = fread(id, 1, 'int');  % # of frames per pass for a ref scan 
hdr.rdb.scnframe = fread(id, 1, 'int');  % total # of frames for a entire scan 
hdr.rdb.pasframe = fread(id, 1, 'int');  % # of frames per pass 
hdr.rdb.user_usage_tag = fread(id, 1, 'uint');  % for spectro 
hdr.rdb.user_fill_mapMSW = fread(id, 1, 'uint');  % for spectro 
hdr.rdb.user_fill_mapLSW = fread(id, 1, 'uint');  % for Spectro 
...
hdr.rdb.ut_ctrl = fread(id, 1, 'int');  % System utility control variable 
...
hdr.rdb.te = fread(id, 1, 'int');  % TE for first echo 
hdr.rdb.te2 = fread(id, 1, 'int');  % TE for second and later echoes 
hdr.rdb.dfmrbw = fread(id, 1, 'float');  % BW for navigator frames 
hdr.rdb.dfmctrl = fread(id, 1, 'int');  % Control flag for dfm (0=off, other=on)
hdr.rdb.raw_nex = fread(id, 1, 'int');  % Uncombined NEX at start of recon 
hdr.rdb.navs_per_pass = fread(id, 1, 'int');  % Max. navigator frames in a pass 
hdr.rdb.dfmxres = fread(id, 1, 'int');  % xres of navigator frames 
hdr.rdb.dfmptsize = fread(id, 1, 'int');  % point size of navigator frames 
hdr.rdb.navs_per_view = fread(id, 1, 'int');  % Num. navigators per frame (tag table) 
hdr.rdb.dfmdebug = fread(id, 1, 'int');  % control flag for dfm debug 
...
hdr.rdb.coilConfigUID = fread(id, 1, 'uint');  
...
hdr.rdb.ectricks_no_regions = fread(id, 1, 'int');  % Total no of regions acquired by PSD 
hdr.rdb.ectricks_input_regions = fread(id, 1, 'int');  % Total no of input regions for reordering 
...
hdr.rdb.numCoilConfigs = fread(id, 1, 'ushort');  % Number of coils to SWIFT between 
...
hdr.rdb.raw_pass_size = fread(id, 1, 'uint64');  
hdr.rdb.sspsave = fread(id, 1, 'uint64');  
hdr.rdb.udasave = fread(id, 1, 'uint64');  
...
for itrans=[1:4]  % kluge for type ChannelTransTableEntryType
  hdr.rdb.cttEntry(itrans).logicalCoilName = fread(id, 128, 'int8=>char')';  % The logicalCoilName of Active Coil
  hdr.rdb.cttEntry(itrans).clinicalCoilName = fread(id, 32, 'int8=>char')';  % The clinicalCoilName of Active Coil
  hdr.rdb.cttEntry(itrans).configUID = fread(id, 1, 'uint32');  % A unique identifier based on coil parameters
  hdr.rdb.cttEntry(itrans).coilConnector = fread(id, 1, 'int32');  % The coilConnector to which Active Coil is connected
  hdr.rdb.cttEntry(itrans).isActiveScanConfig = fread(id, 1, 'uint32');  % Indicates whether this config is used for active scanning
  hdr.rdb.cttEntry(itrans).channelTranslationMap = fread(id, 32, 'int16');  % The channel translation map for this config
end
hdr.rdb.psc_ta = fread(id, 1, 'int32');  
hdr.rdb.disk_acq_ctrl = fread(id, 1, 'int32');  % rhdiskacqctrl control 
hdr.rdb.asset_localTx = fread(id, 1, 'int');  % Set to 1 for local Tx phased arrays 
hdr.rdb.scale3d = fread(id, 1, 'float');  % Use to scale 3D acqs to avoid overrange 
hdr.rdb.broad_band_select = fread(id, 1, 'int');  % Set to 1 for Broadband exciter, Set to 0 for NarrowBand exciter 
hdr.rdb.scanner_mode = fread(id, 1, 'short');  % 1=Product, 2=Research, 3=Service 
hdr.rdb.numbvals = fread(id, 1, 'short');  % Total number of b values for Multi-b DWI 
hdr.rdb.numdifdirs = fread(id, 1, 'short');  % Number of diffusion directions. 1: Single 3: Three 4: Tetrahedral 
hdr.rdb.difnext2 = fread(id, 1, 'short');  % Image-based NEX number for T2 (Smart nex) 
hdr.rdb.difnextab = fread(id, 100, 'short');  % Multi-b eDWI: NEX number for each b-value 
hdr.rdb.channel_combine_method = fread(id, 1, 'short');  % Set to RDB_COMPLEX_COMBINE for complex channel combine recon 
hdr.rdb.nexForUnacquiredEncodes = fread(id, 1, 'short');  % Used for filling NEX table for unacquired encodes 
hdr.rdb.excess = fread(id, 612, 'short');  % free space for later expansion 
hdr.per_pass = fread(id, 2048, 'uint64');  % lumped type RDB_PER_PASS_TAB
hdr.unlock_raw = fread(id, 2048, 'uint64');  % lumped type RDB_PER_PASS_TAB
for islice=[1:2048]  % kluge for type RDB_SLICE_INFO_ENTRY
  ...
  hdr.data_acq_tab(islice).transpose = fread(id, 1, 'short');  % The transpose value for every slice
  hdr.data_acq_tab(islice).rotate = fread(id, 1, 'short');  % The rotate value for every slice
  hdr.data_acq_tab(islice).coilConfigUID = fread(id, 1, 'uint');  % Coil to be used by ASSET for SWIFT type of scans
end
...
hdr.ps.command = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.mps_r1 = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.mps_r2 = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.mps_tg = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.mps_freq = fread(id, 1, 'uint32');  % kluge for type PRESCAN_HEADER
hdr.ps.aps_r1 = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.aps_r2 = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.aps_tg = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.aps_freq = fread(id, 1, 'uint32');  % kluge for type PRESCAN_HEADER
hdr.ps.scalei = fread(id, 1, 'float');  % kluge for type PRESCAN_HEADER
hdr.ps.scaleq = fread(id, 1, 'float');  % kluge for type PRESCAN_HEADER
hdr.ps.snr_warning = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.aps_or_mps = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.mps_bitmap = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.powerspec = fread(id, 256, 'int8');  % kluge for type PRESCAN_HEADER
hdr.ps.filler1 = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.filler2 = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.xshim = fread(id, 1, 'int16');  % kluge for type PRESCAN_HEADER
hdr.ps.yshim = fread(id, 1, 'int16');  % kluge for type PRESCAN_HEADER
hdr.ps.zshim = fread(id, 1, 'int16');  % kluge for type PRESCAN_HEADER
hdr.ps.recon_enable = fread(id, 1, 'int16');  % kluge for type PRESCAN_HEADER
hdr.ps.autoshim_status = fread(id, 1, 'int32');  % to determine whether autoshim was performed or not(smart/OFF)
hdr.ps.rec_std = fread(id, 128, 'float');  % kluge for type PRESCAN_HEADER
hdr.ps.rec_mean = fread(id, 128, 'float');  % kluge for type PRESCAN_HEADER
hdr.ps.line_width = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.ws_flip = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.supp_lvl = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.psc_reuse = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.psc_reuse_string = fread(id, 52, '*char');  % kluge for type PRESCAN_HEADER
hdr.ps.psc_ta = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.phase_correction_status = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.broad_band_select = fread(id, 1, 'int32');  % kluge for type PRESCAN_HEADER
hdr.ps.buffer = fread(id, 64, '*char');  % kluge for type PRESCAN_HEADER
hdr.exam.firstaxtime = fread(id, 1, 'double');  % Start time(secs) of first axial in exam
hdr.exam.double_padding = fread(id, 31, 'double');  % Please use this if you are adding any doubles
...
hdr.exam.float_padding = fread(id, 32, 'float');  % Please use this if you are adding any floats
hdr.exam.ex_delta_cnt = fread(id, 1, 'int');  % Indicates number of updates to header
hdr.exam.ex_complete = fread(id, 1, 'int');  % Exam Complete Flag
hdr.exam.ex_seriesct = fread(id, 1, 'int');  % Last Series Number Used
hdr.exam.ex_numarch = fread(id, 1, 'int');  % Number of Series Archived
hdr.exam.ex_numseries = fread(id, 1, 'int');  % Number of Series Existing
hdr.exam.ex_numunser = fread(id, 1, 'int');  % Number of Unstored Series
hdr.exam.ex_toarchcnt = fread(id, 1, 'int');  % Number of Unarchived Series
hdr.exam.ex_prospcnt = fread(id, 1, 'int');  % Number of Prospective/Scout Series
hdr.exam.ex_modelnum = fread(id, 1, 'int');  % Last Model Number used
hdr.exam.ex_modelcnt = fread(id, 1, 'int');  % Number of ThreeD Models
hdr.exam.int_padding1 = fread(id, 32, 'int');  % Please use this if you are adding any ints
...
hdr.exam.int_padding2 = fread(id, 27, 'int');  % Please use this if you are adding any ints
...
hdr.exam.short_padding = fread(id, 35, 'short');  % Please use this if you are adding any shorts
hdr.exam.hist = fread(id, 257, '*char')';  % Patient History
hdr.exam.refphy = fread(id, 65, '*char')';  % Referring Physician
hdr.exam.diagrad = fread(id, 65, '*char')';  % Diagnostician/Radiologist
hdr.exam.operator_new = fread(id, 65, '*char')';  
...
hdr.exam.ex_padding = fread(id, 240, '*char')';  % Spare Space only for BLOCKs
hdr.series.double_padding = fread(id, 32, 'double');  % Please use this if you are adding any doubles
...
hdr.series.float_padding = fread(id, 32, 'float');  % Please use this if you are adding any floats
hdr.series.se_complete = fread(id, 1, 'int');  % Series Complete Flag
hdr.series.se_numarch = fread(id, 1, 'int');  % Number of Images Archived
hdr.series.se_imagect = fread(id, 1, 'int');  % Last Image Number Used
hdr.series.se_numimages = fread(id, 1, 'int');  % Number of Images Existing
hdr.series.se_delta_cnt = fread(id, 1, 'int');  % Indicates number of updates to header
hdr.series.se_numunimg = fread(id, 1, 'int');  % Number of Unstored Images
hdr.series.se_toarchcnt = fread(id, 1, 'int');  % Number of Unarchived Images
hdr.series.int_padding1 = fread(id, 33, 'int');  % Please use this if you are adding any longs
...
hdr.series.int_padding2 = fread(id, 33, 'int');  % Please use this if you are adding any ints
...
hdr.series.ideal = fread(id, 1, 'short');  % Water, FAT, In-Phase and Out-Phase
hdr.series.short_padding = fread(id, 33, 'short');  % Please use this if you are adding any shots
...
hdr.series.landmark_uid = fread(id, 32+offset, '*char')';  % Landmark Unique ID
...
hdr.series.se_padding = fread(id, 251, '*char')';  % Spare Space
hdr.image.autoSubParam = fread(id, 48, 'char');  % kluge for type AutoSubParam
hdr.image.double_padding = fread(id, 32, 'double');  % Please use this if you are adding any doubles
...
hdr.image.float_padding = fread(id, 33, 'float');  % Please use this if you are adding any floats
hdr.image.cal_fldstr = fread(id, 1, 'uint');  % Calibrated Field Strength (x10 uGauss)
hdr.image.user_usage_tag = fread(id, 1, 'uint');  % Defines how following user CVs are to be filled in
hdr.image.user_fill_mapMSW = fread(id, 1, 'uint');  % Define what process fills in the user CVs, ifcc or TIR
hdr.image.user_fill_mapLSW = fread(id, 1, 'uint');  % Define what process fills in the user CVs, ifcc or TIR
hdr.image.im_archived = fread(id, 1, 'int');  % Image Archive Flag
hdr.image.im_complete = fread(id, 1, 'int');  % Image Complete Flag
hdr.image.int_padding1 = fread(id, 34, 'int');  % Please use this if you are adding any ints
...
hdr.image.obsolete1 = fread(id, 1, 'int');  % Center Frequency (0.1 Hz)
hdr.image.obsolete2 = fread(id, 1, 'int');  % Auto Center Frequency (0.1 Hz)
...
hdr.image.rxmbloblen = fread(id, 1, 'int');  % fMRI: RXM blob size 
hdr.image.rxmblob_pad = fread(id, 1, 'int');  % Changed to 32-bit pad for 64-bit recon build 
hdr.image.im_no = fread(id, 1, 'int');  % Image Number
hdr.image.imgrx = fread(id, 1, 'int');  % Image from which prescribed
hdr.image.temp_phases = fread(id, 1, 'int');  % Temporal Phases
hdr.image.MEG_freq = fread(id, 1, 'int');  % MEG Frequency 
hdr.image.driver_amp = fread(id, 1, 'int');  % Driver Amplitute
hdr.image.driverCyc_Trig = fread(id, 1, 'int');  % Driver Cycle per Trigger
hdr.image.MEG_dir = fread(id, 1, 'int');  % MEG Direction
hdr.image.int_padding2 = fread(id, 26, 'int');  % Please use this if you are adding any ints
...
hdr.image.obsolete3 = fread(id, 1, 'short');  % Actual Transmit Gain (.1 db)
hdr.image.obsolete4 = fread(id, 1, 'short');  % Actual Receive Gain Analog (.1 db)
hdr.image.obsolete5 = fread(id, 1, 'short');  % Actual Receive Gain Digital (.1 db)
...
hdr.image.obsolete6 = fread(id, 1, 'short');  % Auto Transmit Gain (0.1 dB)
hdr.image.obsolete7 = fread(id, 1, 'short');  % PreScan R1 - Analog
hdr.image.obsolete8 = fread(id, 1, 'short');  % PreScan R2 - Digital
...
hdr.image.obsolete9 = fread(id, 1, 'short');  % Auto/Manual Prescan flag
hdr.image.obsolete10 = fread(id, 1, 'short');  % Bitmap of changed values
...
hdr.image.obsolete11 = fread(id, 1, 'short');  % gradient offset in X-direction
hdr.image.obsolete12 = fread(id, 1, 'short');  % gradient offset in Y-direction
hdr.image.obsolete13 = fread(id, 1, 'short');  % gradient offset in Z-direction
...
hdr.image.current_phase = fread(id, 1, 'short');  % Current Phase for this image (DP)
hdr.image.art_level = fread(id, 1, 'short');  % Acoustic reduction level 
hdr.image.slice_group_number = fread(id, 1, 'short');  % value 0=no group defined, 1=this slice belongs to group 1 and so on
hdr.image.number_of_slice_groups = fread(id, 1, 'short');  % value 0=no groups defined, 1=total group is one and so on
hdr.image.show_in_autoview = fread(id, 1, 'short');  % if 1 image will be displayed in auto view 
hdr.image.slice_number_inGroup = fread(id, 1, 'short');  % value 0=no image, 1=this is the 1st image in group and so on
hdr.image.specnuc = fread(id, 1, 'short');  % Imaged Nucleus 1H=> Hydrogen,13C=>carbon,23NA=>Sodium etc
hdr.image.label_duration = fread(id, 1, 'ushort');  % 3DASL: Duration of Lable or Control Pulse
hdr.image.short_padding = fread(id, 39, 'short');  % Please use this if you are adding any shorts
...
hdr.image.rxmpath = fread(id, 64, '*char')';  
hdr.image.psdnameannot = fread(id, 33, '*char')';  % Pulse Sequence Name Annotation 
hdr.image.img_hdr_padding = fread(id, 250, '*char')';  
