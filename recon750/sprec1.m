...
% $HeadURL: svn+ssh://klitinas@woo.engin.umich.edu/work/repos/matlab/recon/branches/phase_zero/sprec1.m $
% $Id: sprec1.m 1266 2014-03-20 18:12:40Z klitinas $
            ...
            for coilnum = 1:ncoils
                ...
                if ncoils > 1
                    ...
                    if args.complex
                        if coilnum == 1
                            imout = (imkb);
                            %fprintf('\rcomplex recon')
                        else
                            % get the phase difference in the middle pixel
                            % between coil images
                            [xdim ydim] = size(imkb);
                            myphs = angle( imkb(xdim/2, ydim/2) / imout(xdim/2, ydim/2));
                            % remove that phase from all of the new coil image so it they
                            % match at that pixel.
                            imout = (imout + exp(-i*myphs)*(imkb));
                        end
                        ...
                        if coilnum == 1
                            imout = abs(imkb);
                            %fprintf('\rmagnitude only recon')
                        else
                            %imout = abs(imout + i.*abs(imkb));
                            imout = imout + i.*abs(imkb);
                        end
                    end
                    imkb = imout;
                end % if ncoils > 1
...
if strcmp(args.outfmt,'nifti')
    ...
    % now the phase
    if (args.complex)
        % p_nii_data(1:2:end) = -p_nii_data(1:2:end);
