function strFileOut = sprec1_3d(pfile,varargin)
% Copyright 2005
	...
	for phnum = stph:enph-1
		...
		for slnum = stsl:ensl-1
			...
			for coilnum = 1:ncoils
				...
				%	LHG:  note the change in teh sum of the coil images		
				if ncoils > 1
			...
			if SHOWME
			figure(2); lightbox(abs(imar)); drawnow;
			end
...
%
		...
		tmp1 = fftshift( fft( fftshift(tmp1,3) ,[], 3) ,3 );
		tmp2 = fftshift( fft( fftshift(tmp2,3) ,[], 3) ,3 );
		tmp3 = fftshift( fft( fftshift(tmp3,3) ,[], 3) ,3 );

		imar = abs(tmp1)+abs(tmp2)+abs(tmp3);
		ahdr.zdim=ahdr.zdim-2;

		if SHOWME
			figure(3); lightbox(abs(imar)); drawnow;
		end
%
