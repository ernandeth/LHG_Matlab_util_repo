function metadata = a75info(filename,varargin)
...
%
%       metadata = a75info('CT_HAND', 'ByteOrder', 'ieee-be');
%
...
fclose(fid);
    ...
    fid = a75open(filename, 'hdr', 'r', ByteOrder);
    ...
    if ~((headerSize >= extendedRange(1)) && ...
        ...
        if (~secondAttempt)
            ...
            fclose(fid);
            eid = 'Images:a75info:hdrFileOpen';
        ...
        if userSupplied.ByteOrder
            msgid = 'Images:a75info:incorrectByteOrder';
        ...
        try
            ...
            msgid = 'Images:a75info:imgFileSize';
    ...
    if isempty(temp)
        if ~alreadyWarned
            msgid = 'Images:a75info:truncatedHeaderFile';
...
if ~isa75(filename)
    eid = 'Images:a75info:invalidAnalyze75file';
...
if (nargin > 1)
    ...
    for k = 1:2:length(varargin)
        ...
        if (isempty(idx))
            eid = 'Images:a75info:unrecognizedParameterName';
            msg = 'Unrecognized parameter name "%s".';
            error(eid, msg, param);
        elseif (length(idx) > 1)
            eid = 'Images:a75info:ambiguousParameterName';
        ...
        switch (paramStrings{idx})
            case 'byteorder'
                if (k == length(varargin))
                    eid = 'Images:a75info:missingByteOrder';
