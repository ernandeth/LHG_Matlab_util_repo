...
% Input Stimulus:  delta functions 380msec. after the onset of the stimulus 
...
for run = 2:7
	...
	index = find( allmids > (run-1)*200  &  allmids < run*200 );
	clean(index) = allmids(index) - shift;
end
...
save regressors6 resp  baseline operation counter operation dual clean
