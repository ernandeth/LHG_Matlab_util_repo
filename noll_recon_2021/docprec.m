% $Id: docprec.m 1266 2014-03-20 18:12:40Z klitinas $
% $HeadURL: svn+ssh://klitinas@anger.engin.umich.edu/svn/matlab/recon/trunk/docprec.m $

if ((scaninfo.revflg > 0) && (args.rsa ~= 2))
    % for reverse spiral
    datin = [];
    for prnum = 1:npr
        ptr = (coilnum-1)*(concat+1)*npr + prnum;
...
if ((scaninfo.revflg ~= 1) && (args.rsa ~= 1))
    ...
    datin = [];
    for prnum = 1:npr
        ptr = (coilnum-1)*(concat+1)*npr + npr*concat + prnum;
