...
% $HeadURL: svn+ssh://klitinas@anger.engin.umich.edu/svn/matlab/recon/trunk/sprec1.m $
% $Id: sprec1.m 1634 2014-12-12 19:59:48Z klitinas $

scriptmode = 1;
if scriptmode == 1
    % in script mode, this is how recon is setup
    %[args,scaninfo,kinfo] = rec_setup1('Pcrush00','ft','sl','l','v');
    % [args,scaninfo,kinfo] = rec_setup1('P14848.7','ft','v');
    %[args,scaninfo,kinfo] = rec_setup1('P20480.7','ft','v');
    % [args,scaninfo,kinfo] = rec_setup1('P02048.7','ft','v','t','1','C','1','d','-290');
    %[args,scaninfo,kinfo] = rec_setup1('P61952.7','ft','v','t','1','s','1','C','0.5','d','2');
   % [args,scaninfo,kinfo] = rec_setup1('P61952.7','ft','v','t','1','s','20','C','0.5','d','2');
    %[args,scaninfo,kinfo] = rec_setup1('P61952.7','ft','v','C','0.5','d','2');
    [args,scaninfo,kinfo] = rec_setup1('P94720.7','ft','v','C','0.5');
else
...
if (args.rtype == 'cp') | (args.rtype == 'ft')
    ...
    kx = []; ky = []; tt = [];
    if scaninfo.transpose == 3
      rotdir = -1;
    else
      rotdir = 1;
    end
    for prnum = 1:npr
        kx = [kx real(kinfo.k.*exp(-i*2*pi*(prnum-1)/npr*rotdir))*OVER];
        ky = [ky imag(kinfo.k.*exp(-i*2*pi*(prnum-1)/npr*rotdir))*OVER];
        tt = [tt kinfo.t];
    end % for prnum = 1:npr
    ...
    wc = wc.*(abs(xx + 1i.*yy) < 0.55*nim)*scfactor*args.outscale;
...
enph = 10;
datsv = zeros(size(dat,1), size(dat,2), enph);
