...
s2 =0.019; % CNR=8; 0.05;  % noise level relative to signal
...
for ARrho =  [0.65 0.75 0.85 0.95]  % iterate over noise level in the data
	...
	for h1 = linspace(0.01,3,20)
		...
		for count=1:50  % iterations for ROC samples
            ...
            %n = sqrt(s2)/sqrt(1+ARrho^2)*randn(T,1);
			s2eta=2.3/0.4*s2;
            w=filter(1-ARrho,[1 -ARrho],sqrt(s2eta)*randn(T,1));
            v=sqrt(s2)*randn(T,1);
            n=w+v;
            n=n*sqrt(0.12)/std(n); % Fix CNR at 8
...
legend('\rho = 0.65', '\rho = 0.75', '\rho = 0.85','\rho = 0.95')
line([0 1], [0 1])
