...
voltage = 10; %input('What is the voltage of the nongrounded side?');
boxside = 10% input('Number of units on side of box? ');

% Set the maximum allowable change in value for a point between iterations
tolerance = 0.01;%input('tol ');	

% The next line initializes the grid, setting all points to zero volts
old = zeros(boxside,boxside);
    old(boxside/2:boxside/2+1, boxside/2:boxside/2+1) = voltage;

%old(boxside/2, boxside/2) = voltage;
...
while (equilibrium < 1)
    ...
    new(boxside/2, boxside/2) = voltage;
