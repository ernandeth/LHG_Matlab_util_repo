function y = ginx2spm2(dim1,dim2,slc1,slc2);
...
% Spun off of ginx2spm1.m with changes to autoscale to short int.
% TLChenevert 3/98.
...
% Display some images if desired 
 if(phi == ph1)
   ...
   maxpix = max(max(max(y)));  % Autoscale to 30k
   scaleup = 30000/maxpix;   
 end % if phi
