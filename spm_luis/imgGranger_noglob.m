function imgGranger(rootname, xyz, DesMat,Nlag)
% function imgGranger(rootname, xyz_coords,DesMat, Nlag)
%
...
subplot(221)
hist(Fab(:),100);title('Granger F_A->_B')
%axis([-0.2 0.2 0 50])
subplot(222)
hist(Fba(:),100);title('Granger F_B->_A')
%axis([-0.2 0.2 0 50])
subplot(223)
hist(F(:),100); title('Granger F (Fab-Fba)')
subplot(224)
hist(pval(:),100);title('P(F)')

fprintf('\n writing results ... Fab - Fba\n')
global SPM_scale_factor
SPM_scale_factor = 1/100;
h.tdim=1;
write_hdr('grangerlog10P.hdr',h);
write_img('grangerlog10P.img',-log10(pval)*100,h);
write_hdr('grangerFab.hdr',h);
write_img('grangerFab.img',Fab*100,h);
write_hdr('grangerFba.hdr',h);
write_img('grangerFba.img',Fba*100,h);
write_hdr('grangerF.hdr',h);
write_img('grangerF.img',F*100,h);
write_hdr('grangerCHI2.hdr',h);
write_img('grangerCHI2.img',chi2*100,h);
