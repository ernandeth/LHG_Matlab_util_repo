%these simulations are to check how harmful the variation of HRF is to
%Granger causality
...
%     end
    ...
    %hrf_a = spm_hrf(TR, [ tau(1) 16 1 1 6 0 32]);
    %hrf_b = spm_hrf(TR, [ tau(2) 16 1 1 6 0 32]);
    ...
    a = a + noise* V *randn(size(a)) ;
    b = b + noise* V *randn(size(b)) ;
...
%     b_events = indicator(b, threshold, binsize);
    ...
    if doPlots  & iter==1
        figure(1)
        plot(100*a,'r'), hold on, plot(100*b)
        stem(ae1,'r'), stem(be1,'--b')
        title('Sample FMRI Time Courses ')
        legend('A', 'B')
...
%    all_dom(iter) = D;
  ...
  %  all_G_inst(iter) = grangerF_inst;
    ...
    fprintf('\riteration: %d  G= %2.3ff',iter, grangerF);
end
...
figure(4)
subplot(311), hist(all_Fab,20,'k'), title(sprintf('mean F_{ab} = %0.3f', mean(all_Fab))), dofontsize(16), axis([0 1.5 0 200])
%subplot(322), hist(all_Fab_inst,20,'k'), title(sprintf('mean F_{ab inst} = %0.3f', mean(all_Fab_inst))), dofontsize(16), axis([0 1.5 0 200])
subplot(312), hist(all_Fba,20,'k'), title(sprintf('mean F_{ba} = %0.3f', mean(all_Fba))), dofontsize(16), axis([0 1.5 0 200])
%subplot(314), hist(all_Fba_inst,20,'k'), title(sprintf('mean F_{ba inst} = %0.3f', mean(all_Fba_inst))), dofontsize(16), axis([0 1.5 0 200])
subplot(313), hist(all_G,20,'k'), title(sprintf('mean GF = %0.3f', mean(all_G))), dofontsize(16), axis([-1 1 0 200])
