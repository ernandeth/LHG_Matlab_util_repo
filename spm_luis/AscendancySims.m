...
all_asc = zeros(1,NITER);
all_thetas = zeros(4,NITER);
...
figure
subplot 311 , hist(all_rho,20) , title('Correlation'), dofontsize(16)
subplot 312 , hist(all_mi,20),  title('Mutual Information'), dofontsize(16)
subplot 313 , hist(all_dom,20), title('Dominance: P(B|A) - P(A|B)'), dofontsize(16)
%}
figure
meanDom = zeros(1,NITER/NXITER);
stdDom = zeros(1,NITER/NXITER);
for xtra=0:9
    buf = all_dom(xtra*NXITER +1 : (xtra+1)*NXITER);
    buf = buf(~isnan(buf));
    meanDom(xtra+1) = mean(buf);
    stdDom(xtra+1) = std(buf);
end

x = [0:9];
errorbar((10+x)/10 , meanDom, stdDom,stdDom, 'ok')
hold on
x = [0:0.1:1];
y = x./(1+x);
plot((1+x)/1 ,y, 'k-') 
xlabel('Number of B events independent of A')
ylabel('Dominance')
title('Effect of independent events fraction') 
legend('Ideal spike data', 'Noisy BOLD data')
fatlines
dofontsize(16)
axis([1 2 -0.3 0.5])
print -dtiff Dom_Nextra

save simulations_extraEvs
