function strFileOut = sprec1_rds_despiked(pfile,varargin)
% Copyright 2005
...
% $HeadURL: svn+ssh://klitinas@woo.engin.umich.edu/work/repos/matlab/recon/trunk/sprec1_rds.m $
% $Id: sprec1_rds_despiked.m 1700 2015-04-13 15:06:14Z klitinas $
...
if scriptmode == 1
    ...
    if exist('pfile','var') == 0
        %rec_setup1;
        rec_setup1_streaming_dv24
        return
    else
        [strPath,strName,strExt] = fileparts(pfile);
        if strcmpi(strExt,'.data')
            strFilePrep = fullfile(strPath,[strName '.prep']);
            [args,scaninfo,kinfo] = rec_setup1_streaming_dv24(strFilePrep,varargin{:});
        else
...
if (args.map == 1)
    ...
    for slnum = stsl:ensl-1
        ...
        if strcmpi(strExt,'.data')
%             dat0 = loaddat_rds(0,slnum,scaninfo,fid);
%             dat1 = loaddat_rds(1,slnum,scaninfo,fid);
            dat0 = getdespikeddat(0,slnum,scaninfo,pfile);
            dat1 = getdespikeddat(1,slnum,scaninfo,pfile);
        else
    ...
    for phnum = stph:enph-1
        ...
        for slnum = stsl:ensl-1
            ...
            if strcmpi(strExt,'.data')
                %dat = loaddat_rds(phnum,slnum,scaninfo,fid);
                dat = getdespikeddat(phnum,slnum,scaninfo,pfile);
            else
        ...
        basestr = sprintf('vol_%s_%04d',datestr,phnum+args.imoffset);
        if args.svraw == 1
