...
% $Id: rec_setup1_with_compression.m 1433 2014-06-17 20:11:57Z klitinas $
% $HeadURL: svn+ssh://klitinas@woo.engin.umich.edu/work/repos/matlab/recon/trunk/rec_setup1_with_compression.m $
...
if exist('rawfile','var') == 0
  ...
  fprintf(stderr,'\n\n Martin added the following functions\n');
  fprintf(stderr,'ComPress #         Compress the data into # virtual coils\n');
  fprintf(stderr,'ComMfrom #         Calculate the compression matrix from temporal frame #\n');
  fprintf(stderr,'saveComM strname   Save ComM to file strname\n');
  fprintf(stderr,'loadComM strname   Load ComM from file strname\n');
  fprintf(stderr,'ComMonly strname   Calculate ComM only (no recon) and save it to file strname\n');
  fprintf(stderr,'AddNoise           Add noise (need to be set in sprec1) to ksapce data');
  return
end
%
...
if exist('args','var') == 0
    ...
    args.complex = 0;   
     ...
     %-----------------------------------------------------------------------
    args.compression = 0;    %%MARTIN ADDED IT!!! He might messed this up
    args.ComMfrom = 1;
    args.saveComM = 0;
    args.loadComM = 0;
    args.ComMonly = 0;
...
%      case 'O', args.pt = getnum(cell2mat(varargin(argn))); argn = argn+1;
 ...
 %---------------------------------------------------------------        
       %%MARTIN ADDED IT!!! He might messed this up   
      case 'ComPress', args.compression = 1;
          args.rdncoil = getnum(cell2mat(varargin(argn))); 
          % the number of virtual coils
          argn = argn+1;
          
      case 'ComMfrom',
          args.ComMfrom = getnum(cell2mat(varargin(argn)));
         % the ph from which compression matrix is calculated from
          argn = argn+1;
          
          
      case 'saveComM', args.saveComM = 1;
          args.saveComMname = cell2mat(varargin(argn));
          % the name of file to store the compression matrix
          argn = argn+1;
      
      case 'loadComM', args.loadComM = 1;
          args.loadComMname = cell2mat(varargin(argn));
          % the name of file from which the compression matrix is retreived
          argn = argn+1;  
          
       case 'ComMonly', args.ComMonly = 1; args.saveComM = 1;
           args.saveComMname = cell2mat(varargin(argn));
          % the name of file to store the compression matrix
          argn = argn+1;
          
        case 'AddNoise', args.ifAddNoise = 1;
