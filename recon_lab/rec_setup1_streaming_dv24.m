    function [args,scaninfo,kinfo] = rec_setup1_streaming_dv24(prepfile,varargin)
%function [args,scaninfo,kinfo] = rec_setup1(rawfile,varargin)
%

% $Id: rec_setup1_streaming_dv24.m 1698 2015-04-13 14:57:53Z klitinas $
...
% error message
%
if exist('prepfile','var') == 0
  fprintf(stderr,'Usage: %s prepfile [OPTIONS]\n',mfilename);
  fprintf(stderr,'\nRecon Options\n');
...
%      case 'O', args.pt = getnum(cell2mat(varargin(argn))); argn = argn+1;
      ...
      case 'cm', args.cpmap = 1;    % does conj phase correction on existing fieldmap (fmfile.mat)
      otherwise, 
        fprintf(stderr,'%s: Did not recognize option %s\n',mfilename,argtype);
    end % switch
  end % while

hdr.rdb = ge_readhdr_streaming_prepfile(prepfile);
