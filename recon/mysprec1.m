function sprec1(pfile,varargin)
% Copyright 2005
...
if (args.map == 1)
    ...
    for phnum = stph:enph-1
        ...
        if strcmp(args.outfmt,'nifti')
            ...
            % LHG - add phase data handling.
            if args.complex==1
                niidata_phs(nii_start:nii_stop) =  angle(imar(:));
            end
        ...
        if strcmp(args.outfmt, 'anal')
            ...
            % LHG - add phase data handling.
            if args.complex==1
                write_hdr(['p_' basestr '.hdr'],ahdr);
                write_img(['p_' basestr '.img'],1000*angle(imar),ahdr);
            end  
...
if strcmp(args.outfmt,'nifti')
    ...
    % LHG - add phase data handling.
    if args.complex==1
        write_nii(sprintf('p_%s.nii',basestr), niidata_phs*1000, nhdr,0);
    end
end
