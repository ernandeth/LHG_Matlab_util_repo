function slice = rt_sprec1(pfile,varargin)
% Real Time version of sprec1:
%
% For RT mode, call it as
%
%           rt_sprec1(inBuffer, args, scaninfo, kinfo)
%
% Note - those info arguments are generated by rt_rec_setup1
%
% Luis hernandez-garcia
% modifications to sprec1
%
% from
% Copyright 2005
...
%
% call rt_sprec1 with no arguments to find options
%
...
if scriptmode == 1
    ...
    [args,scaninfo,kinfo] = rec_setup1(pfile,'ft','sl','l','v');
else
    ...
    if ~exist('pfile','var')
        ...
        if ~isstr(pfile)
            ...
            inBuffer = reshape(pfile,  length(pfile),1);
...
if (args.map == 1)
    ...
    % phase loop is the time frames loop
    for phnum = stph:enph-1
        ...
        for slnum = stsl:ensl-1
            ...
            %%%%
            %%% THIS IS WHERE WE STICK IN THE REAL TIME DATA
            %%%%
            if RTMODE
                dat = complex(inBuffer(1:2:end) , inBuffer(2:2:end));
            else
            ...
            for coilnum = 1:ncoils
                ...
                %%%%%%%%%%%%%%%%%%
                %%%% THIS IS THE HEART OF THE RECON
                %%%%%%%%%%%%%%%%%%%%%%
                % do ft or cp recon
                docprec;
        ...
        if RTMODE
            % in real time, we don't worry about writing files yet!
            slice = imar;
