function delay_calc_abbreviated_summary(strFile,dIn,dOut,rdat,ddat,zdiff,sliceToShow,strFileOut)
% basically same as delay_calc_summary used in rotation stability, but only
% shows delay terms -5:5.
% 
% $Id: delay_calc_abbreviated_summary.m 1483 2014-08-13 14:19:08Z klitinas $
% $HeadURL: svn+ssh://klitinas@anger.engin.umich.edu/svn/matlab/img/trunk/delay_calc_abbreviated_summary.m $
...
hText = text(1,-24,strText,'interpreter','none');
...
show(tile(squeeze(ddat(:,:,sliceToShow,16:26))));
set(hAxDdat,'xticklabel','');
title('ddat (reverse)');

hAxRdat = subplot(324);
% show(tile(squeeze(rdat(:,:,sliceToShow,:))));
show(tile(squeeze(rdat(:,:,sliceToShow,16:26))));
xlabel('rdat (forward)');
...
hSqIn = rectangle('position',[70 70 55 55],'edgecolor',[0 0 1],'linewidth',2);


axes(hAxRdat);
hold on;
hSqOut = rectangle('position',[70 70 55 55],'edgecolor',[1 0 0],'linewidth',2);

%set(hAxZdiff,'pos', [0.1    0.1    0.2722    0.2649]);
set(hAxZdiff,'pos', [0.1    0.1    0.20    0.20]);
set(hAxZeroR,'pos',[0   0.35    0.3347    0.2157])
%set(hAx_ddatDelay0,'pos',[0.0 0.7093 0.3347    0.2157])
