...
% $Id: analyze75read.m 1279 2014-03-24 20:06:25Z klitinas $
% $HeadURL: svn+ssh://klitinas@anger.engin.umich.edu/svn/matlab/img/trunk/analyze75read.m $
...
fid = a75open(ImgFilename, 'img', 'r', ByteOrder);
...
switch class(FirstArg)
    ...
    otherwise
        eid = 'Images:a75read:invalidInputArgument';
...
try
    ...
    if userSupplied.metadata
        oldmsgstr = lasterr;
        eid = 'Images:a75read:invalidMetadataStruct';
...
switch ImgDataType
    ...
    otherwise 
        ...
        eid = 'Images:a75read:unsupportedImageDatatype';
...
if isempty(temp)
    ...
    % generate an error.
    fclose(fid);
    eid = 'Images:a75read:truncatedImageFile';
